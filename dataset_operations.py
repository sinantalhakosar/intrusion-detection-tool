        ...
        if df[col_name].dtypes == 'object':
            if col_name != " Label":
                for i in range(0,len(df[col_name].values)):
                    print ("\r Processing... {:.2f}".format(i*100./(df.shape[0]))+str("%"), end="")
                    try:
...
def label_replace(df):
    labeldf = df[" Label"]
    newlabeldf=labeldf.replace({v:k for k, v in enumerate(labeldf.unique())})
    df[" Label"] = newlabeldf
    return df

def dataset_split_and_label_replace(df):
